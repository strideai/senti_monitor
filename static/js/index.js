...
app.controller('sentiment_manager', ['$scope', '$http', function($scope, $http, entityList){
...
app.controller('comparison_manager', ['$scope', '$http', function($scope, $http, entityList){
	$scope.expectedArticleList = [], $scope.order = 'null', $scope.topEntityList = [], $scope.result = true, $scope.entityFilter = '', $scope.comparecount = 0, $scope.compareEntityList = [], $scope.compareResult=[];

	$scope.textChange = () => {
		if ($('#entityFinder').val() != "") {
			$('#entityFinder').addClass('filled');
		} else {
			$('#entityFinder').removeClass('filled');
		}
	}
	...
	$scope.addEntity = () => {
		$scope.comparecount++;
		if($scope.compareEntityList.length>2)
			$scope.compareEntityList.shift();
		$scope.compareEntityList.push($scope.compareEntities.title);
		angular.element("#entityFinder").val('');
		// console.log($scope.compareEntityList);
		$http({
			method: 'GET',
			url: '/compare?entities='+$scope.compareEntityList.join(',')
		})
		.then((resp) => {
			$scope.compareResult = resp.data;
		})
	}
